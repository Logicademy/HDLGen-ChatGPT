...
-- entity declaration
entity reg4x4_TB is end entity reg4x4_TB;

architecture behave of reg4x4_TB is
...
UUT: reg4x4-- map component internal sigs => testbench signals
port map
	(
	clk => clk, 
	rst => rst, 
...
-- manually added code START
   ...
   -- individual tests. Generate input signal combinations and wait for period.
	...
	testNo <= 2; -- write all mem locations
	ce <= '1';
    we <= '1';
    for i in 0 to 3 loop
 	 dIn <= std_logic_vector( to_unsigned(i, 4) + 1 );
	 add <= std_logic_vector( to_unsigned(i, 2) );
	 wait for period;
	end loop;

	testNo <= 3; -- write mem(1) = 8, though ce = 0 so not write occurs 
	ce <= '0';
	we <= '1';
	dIn <= X"9";
	add <= "01";
	...
	testNo <= 4; -- combinational read all locations
	ce <= '0';
    we <= '0';
    for i in 0 to 3 loop
 	 dIn <= X"3";
	...
	testNo <= 5; -- deassert input signals
	ce <= '0';
	we <= '0';
	dIn <= (others => '0');
	add <= (others => '0');
